...
import cv2
# if the code does not run, please change the file name with the full file path
face = cv2.CascadeClassifier('face.xml')
# face = cv2.CascadeClassifier('C:/face.xml')
    ...
    while 1:
        ...
        for (x, y, w, h) in item:
            ...
            if xx < 0.5 * ww and xx != 0:
                cv2.putText(pic, 'towards left', (x, y + h // 3),
                            font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
            elif xx > 0.8 * ww and xx != 0:
                cv2.putText(pic, 'towards right', (x, y + h // 3),
                            font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)

            if yy < 0.3 * hh and yy != 0:
                cv2.putText(pic, 'towards top', (x, y + h * 2 // 3),
                            font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
            elif yy > 0.5 * hh and yy != 0:
                cv2.putText(pic, 'towards bottom', (x, y + h * 2 // 3),
                            font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
            text = 2500.0 / w * 3.66
            text = int(text)
            txt = str(text) + "cm Far"
            # cv2.putText(pic, 'Target @ Centre',(x, y+h/2), font, 0.8, (255, 0, 0), 1, cv2.LINE_AA)
            cv2.putText(pic, txt, (x+10, y + h // 2), font,
